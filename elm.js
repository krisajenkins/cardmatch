...
Elm.API.make = function (_elm) {
   ...
   var decodeDimensions = A2($Json$Decode$Extra.apply,
   A2($Json$Decode.map,
   $Schema.Dimensions,
   A2($Json$Decode._op[":="],
   "w",
   ...
   "url",
   $Json$Decode.string));
   var decodeVariant = A2($Json$Decode.map,
   $Schema.Variant,
   A2($Json$Decode._op[":="],
   "images",
   $Json$Decode.list(decodeImage)));
...
Elm.Config.make = function (_elm) {
   ...
   var guessDuration = 2 * $Time.second;
   var gameDuration = 3 * $Time.minute;
...
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm),
   $UI = Elm.UI.make(_elm);
   var newGame = F3(function (seed,
   time,
   ps) {
      return function () {
         var productIds = $Dict.keys(ps);
         var gameSize = A2($Basics.min,
         6,
         $List.length(productIds));
         var lefts = A3($List.map2,
         F2(function (v0,v1) {
            return {ctor: "_Tuple2"
                   ,_0: v0
                   ,_1: v1};
         }),
         A2($List.repeat,gameSize,0),
         A2($List.take,
         gameSize,
         productIds));
         var rights = A3($List.map2,
         F2(function (v0,v1) {
            return {ctor: "_Tuple2"
                   ,_0: v0
                   ,_1: v1};
         }),
         A2($List.repeat,gameSize,1),
         A2($List.take,
         gameSize,
         productIds));
         return {ctor: "_Tuple2"
                ...
                ,_1: {_: {}
                     ...
                     A2($List.append,lefts,rights))
                     ,duration: $Config.gameDuration
                     ,guesses: $Set.empty
                     ,started: time}};
      }();
   });
   var maybeNewGame = F3(function (seed,
   time,
   products) {
      return function () {
         switch (products.ctor)
         {case "Success":
            return $Maybe.Just(A3(newGame,
              seed,
              time,
              products._0));}
         return $Maybe.Nothing;
   ...
   model) {
      return function () {
         ...
         {case "LoadProducts":
            ...
            case "NewGuess":
            return A2($System.fmapGame,
              $System.recordGuess({ctor: "_Tuple2"
                                  ,_0: model.currentTime
                                  ,_1: action._0}),
              model);
            case "NoOp": return model;
            case "Tick":
            return A2($System.fmapGame,
              $System.expireGuesses(action._0),
              _U.replace([["currentTime"
                          ,action._0]],
              model));}
         _U.badCase($moduleName,
   ...
   var currentTimeSignal = $Time.every(100 * $Time.millisecond);
   var modelSignals = $Signal.mergeMany(_L.fromArray([uiMailbox.signal
                                                     ,A2($Signal._op["<~"],
                                                     $System.LoadProducts,
                                                     $API.products)
                                                     ,A2($Signal._op["<~"],
   ...
   var startSeed = $Random.initialSeed($Basics.round(startTime));
   var model = A3($Signal.foldp,
   step,
   A2($System.initialModel,
   startTime,
   startSeed),
   modelSignals);
...
Elm.Schema.make = function (_elm) {
   ...
   var Variant = function (a) {
      return {_: {},images: a};
   };
   var Image = F2(function (a,b) {
      return {_: {}
             ,dimensions: a
             ,url: b};
   });
...
Elm.System.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $Time = Elm.Time.make(_elm);
   var initialModel = F2(function (t,
   seed) {
      return {_: {}
             ,currentGame: $Maybe.Nothing
             ,currentTime: t
             ,points: 0
             ,products: $Http.NotAsked
             ,randomSeed: seed};
   });
   var markShowing = F2(function (cardIds,
   card) {
      return A2($Set.member,
      $Basics.fst(card),
      cardIds) ? {ctor: "_Tuple2"
                 ,_0: $Basics.fst(card)
                 ,_1: true} : card;
   });
   var isCorrect = function (guesses) {
      return _U.eq($List.length($Set.toList(guesses)),
      2) && _U.eq($List.length($Set.toList(A2($Set.map,
      function ($) {
   ...
   var recordGuess = F2(function (guess,
   game) {
      return function () {
         var newGuesses = A2($Set.insert,
         guess,
         game.guesses);
         return _U.cmp($List.length($Set.toList(newGuesses)),
         2) > 0 ? game : isCorrect(newGuesses) ? _U.replace([["guesses"
                                                             ,$Set.empty]
                                                            ,["cards"
                                                             ,A2($List.map,
                                                             markShowing(A2($Set.map,
                                                             $Basics.snd,
                                                             newGuesses)),
                                                             game.cards)]],
         game) : _U.replace([["guesses"
                             ,newGuesses]],
         game);
      }();
   });
   var isOld = F2(function (expires,
   guess) {
      return function () {
         var _ = guess;
         var time = function () {
         ...
         return _U.cmp(expires,
         time + $Config.guessDuration) < 0;
      }();
   });
   var expireGuesses = F2(function (t,
   game) {
      return function () {
         var expiredGuesses = A2($Set.filter,
         isOld(t),
         game.guesses);
         return _U.replace([["guesses"
                            ,_U.eq(expiredGuesses,
                            $Set.empty) ? $Set.empty : game.guesses]],
         game);
      }();
   });
   var isActive = F2(function (guesses,
   card) {
      return function () {
         var $ = card,
         cardId = $._0,
         showing = $._1;
         return showing || A2($Set.member,
         cardId,
         A2($Set.map,
         $Basics.snd,
         guesses));
   ...
   var isComplete = function (game) {
      return function () {
         var showings = A2($List.map,
         $Basics.snd,
         game.cards);
         return A3($List.foldl,
         F2(function (x,y) {
   ...
   var fmapGame = F2(function (f,
   model) {
      return _U.replace([["currentGame"
                         ,A2($Maybe.map,
                         f,
                         model.currentGame)]],
      model);
   });
   var NewGuess = function (a) {
      return {ctor: "NewGuess"
             ,_0: a};
   };
   var NewGame = {ctor: "NewGame"};
   var LoadProducts = function (a) {
      return {ctor: "LoadProducts"
             ,_0: a};
   };
   var Tick = function (a) {
...
Elm.UI.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $System = Elm.System.make(_elm),
   $UICommon = Elm.UICommon.make(_elm);
   var winnerView = function (uiChannel) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("col-xs-12 col-sm-4 col-sm-offset-4")]),
      _L.fromArray([A2($Html.h1,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("You Win!")]))
                   ,A2($Html.p,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Congratulations! Use this code to receive 10% off your next purchase!")]))
                   ,A2($Html.h4,
                   _L.fromArray([$Html$Attributes.$class("offer-code")]),
                   _L.fromArray([$Html.text("Code: CARDFLIPPER")]))
                   ,A2($Html.button,
                   _L.fromArray([$Html$Attributes.$class("btn btn-primary btn-block btn-lg")
                                ,A2($Html$Events.onClick,
                                uiChannel,
                                $System.NewGame)]),
                   _L.fromArray([$Html.text("Play Again")]))]))]));
   };
   var productImage = function (p) {
      return function () {
         var _v0 = $List.head(p.masterVariant.images);
         switch (_v0.ctor)
         {case "Just":
            return A2($Html.div,
              _L.fromArray([$Html$Attributes.$class("product-image")
                           ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                 ...
                                                                 _v0._0.url,
                                                                 ")"))}]))
                           ,$Html$Attributes.width(_v0._0.dimensions.width)
                           ,$Html$Attributes.height(_v0._0.dimensions.height)]),
              _L.fromArray([]));
            case "Nothing":
            return $UICommon.loadingView;}
         _U.badCase($moduleName,
         "between lines 69 and 75");
      }();
   };
   var cardView = F4(function (uiChannel,
   products,
   guesses,
   card) {
      return function () {
         ...
         var cardId = function () {
            ...
            {case "_Tuple2": return _._0;}
            _U.badCase($moduleName,
            "on line 55, column 20 to 24");
         }();
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("col-xs-4 col-sm-2")]),
         function () {
            var _v5 = A2($Dict.get,
            $Basics.snd(cardId),
            products);
            ...
            {case "Just":
               return _L.fromArray([A2($Html.div,
                 _L.fromArray([$Html$Attributes.classList(_L.fromArray([{ctor: "_Tuple2"
                                                                       ...
                                                                       ,{ctor: "_Tuple2"
                                                                        ,_0: "active"
                                                                        ,_1: A2($System.isActive,
                                                                        guesses,
                                                                        card)}]))
                              ,A2($Html$Events.onClick,
                              uiChannel,
                              $System.NewGuess(cardId))]),
                 _L.fromArray([A2($Html.div,
                              _L.fromArray([$Html$Attributes.$class("back")]),
                              _L.fromArray([]))
                              ,A2($Html.div,
                              _L.fromArray([$Html$Attributes.$class("face")]),
                              _L.fromArray([productImage(_v5._0)]))]))]);
               case "Nothing":
               return _L.fromArray([$UICommon.errorView("Product Not Found")]);}
            _U.badCase($moduleName,
            "between lines 57 and 65");
         }());
      }();
   });
   var gameView = F3(function (uiChannel,
   products,
   game) {
      return A2($Html.div,
      _L.fromArray([]),
   ...
   var startGameView = function (uiChannel) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("col-xs-12 col-sm-4 col-sm-offset-4")]),
      _L.fromArray([A2($Html.h1,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Cape Match")]))
                   ,A2($Html.button,
                   _L.fromArray([$Html$Attributes.$class("btn btn-primary btn-block btn-lg")
                                ,A2($Html$Events.onClick,
                                uiChannel,
   ...
   var mainContentView = F2(function (uiChannel,
   model) {
      return function () {
         var _v7 = {ctor: "_Tuple2"
                   ,_0: model.products
                   ,_1: model.currentGame};
         switch (_v7.ctor)
         {case "_Tuple2":
            switch (_v7._0.ctor)
              {case "Failure":
                 return $UICommon.errorView(_v7._0._1);
                 case "NotAsked":
                 return $UICommon.loadingView;
                 case "Success":
                 switch (_v7._1.ctor)
                   {case "Just":
                      return $System.isComplete(_v7._1._0) ? winnerView(uiChannel) : A3(gameView,
                        uiChannel,
                        _v7._0._0,
                        _v7._1._0);
                      case "Nothing":
                      return startGameView(uiChannel);}
                   break;
                 case "Waiting":
                 return $UICommon.loadingView;}
              break;}
   ...
   model) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.id("root-view")
                   ,$Html$Attributes.$class("container-fluid")]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("col-xs-12 col-md-10 col-md-offset-1")]),
      _L.fromArray([A2(mainContentView,
      uiChannel,
      model)]))]))]));
   });
...
Elm.UICommon = Elm.UICommon || {};
Elm.UICommon.make = function (_elm) {
   "use strict";
   _elm.UICommon = _elm.UICommon || {};
   if (_elm.UICommon.values)
   return _elm.UICommon.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var errorView = function (message) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("alert alert-error")]),
      _L.fromArray([A2($Html.h3,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Something went wrong!")]))
                   ,$Html.text(message)]));
   };
